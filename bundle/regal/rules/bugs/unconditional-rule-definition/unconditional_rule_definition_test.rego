...
# `else` is the supported way to write a fallback. It is one rule with an
# ordered chain, not a set of definitions, so the earlier conditions do decide
# the answer and there is nothing to report.
...
test_success_ref_with_variable_part if {
	r := rule.report with input as ast.with_rego_v1(`
	config[k] := "loose"

	config[k] := "strict" if input.strict[k]
	`)
